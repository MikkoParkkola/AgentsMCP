...
class Orchestrator:
    ...
    async def process_user_input(self, user_input: str, context: Optional[Dict] = None) -> OrchestratorResponse:
        ...
        try:
            ...
            logger.debug(f"Task classified as: {classification.classification.value} "
    ...
    async def _handle_single_agent_task(self, user_input: str, context: Optional[Dict],
        ...
        logger.info(f"Delegating to single agent: {classification.required_agents}")
        
        try:
            # Get the recommended agent
            agent_type = classification.required_agents[0] if classification.required_agents else "codex"
