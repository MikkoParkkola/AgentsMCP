...
class RAGPipeline:
    ...
    async def add_documents_from_directory(self, directory_path: str, patterns: List[str] = None):
    ...
    async def retrieve(self, query: str, max_results: int = None) -> List[RetrievalResult]:
    ...
    async def _embedding_retrieval(self, query: str, max_results: int) -> List[RetrievalResult]:
        ...
        for document in self.documents.values():
            if document.embedding:
                ...
                if similarity >= self.rag_config.similarity_threshold:
                    results.append(RetrievalResult(document=document, score=float(similarity)))
        
        # Sort by score descending
        results.sort(key=lambda x: x.score, reverse=True)
        return results[:max_results]
    
    async def _keyword_retrieval(self, query: str, max_results: int) -> List[RetrievalResult]:
    ...
    def _chunk_text(self, text: str) -> List[str]:
        ...
        for i in range(0, len(words), self.rag_config.chunk_size - self.rag_config.chunk_overlap):
            chunk_words = words[i:i + self.rag_config.chunk_size]
